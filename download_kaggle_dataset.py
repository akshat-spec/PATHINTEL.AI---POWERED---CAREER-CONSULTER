...
def create_synthetic_dataset():
    ...
    for template in templates:
        for i in range(template['variations'] * 70): # Increased variation multiplier by 70x
    ...
    for resume, job, score in mismatch_pairs:
        for _ in range(240):  # Increased to 240 variations each
            noise = np.random.normal(0, 5)
    ...
    for resume, job, score in medium_templates:
        for _ in range(300): # Increased to 300 variations each
